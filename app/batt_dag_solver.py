...
def calculate_bottleneck_cost(batteries, graph, rssi, bottleneck_dummy):
    comm_cost_finder = functools.partial(find_comm_cost, graph,rssi)
    battery_cost_finder = functools.partial(cost_finder_to_batt, comm_cost_finder, batteries)
    return min(battery_cost_finder(bottleneck_dummy.one),
               battery_cost_finder(bottleneck_dummy.two))
...
def find_comp_cost(graph, processors, a_dummy):
    ...
    return comp_cost
def find_cost(graph,batteries, processors, rssi, a_dummy):
...
def inout_consistency(graph, dummies, p, dummy_vars):
    ...
    return p
def formulate_LP(graph, constraints, processors, rssi, batteries):
    ...
    cost_calculator = functools.partial(find_cost, graph,batteries, processors, rssi)
    ...
    bottleneck_cost_calculator = functools.partial(calculate_bottleneck_cost,batteries, graph, rssi)
